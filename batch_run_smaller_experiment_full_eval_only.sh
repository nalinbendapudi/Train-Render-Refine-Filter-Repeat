...
do
    ...
    # # GS_br_c (Train further with corrupted versions of bad renders from 30k to 40k)
    # printf "\n\n----------------------------------"
    # printf "\nGS_br_c (Train further with corrupted versions of bad renders) for: %s\n" "$SCENE"

    # CUDA_VISIBLE_DEVICES=0 python examples/gsplat/simple_trainer_difix3d.py default \
    # --data_dir ${DATA_DIR} \
    # --data_factor ${DATA_FACTOR} \
    # --result_dir ${RESULT_PARENT_DIR}/${NOVEL_STRATEGY_FOLDER_NAME}/GS_br_c/ \
    # --no-normalize-world-space \
    # --test_fraction ${TEST_FRACTION} \
    # --init_type ${INIT_TYPE} \
    # --ckpt ${RESULT_PARENT_DIR}/${NOVEL_STRATEGY_FOLDER_NAME}/GS_br_c/ckpts/ckpt_39999_rank0.pt \
